...
def generate_xml(annotations, pic_name, WIDTH, HEIGHT):
	#make copy of base
	shutil.copy("base.xml", "{}/{}.xml".format(os.getcwd(),pic_name))
	#assign filename to boiler plate
	xml_read = open(("{}/{}.xml".format(os.getcwd(),pic_name)), 'r')
	...
	xml_write = open(("{}/{}.xml".format(os.getcwd(),pic_name)), 'w')
	xml_write.writelines(new_lines)
	xml_write.close()

	xml_append = open(("{}/{}.xml".format(os.getcwd(),pic_name)), 'a')
	for a in annotations: #start appending in the objects we found
		...
		for line in object_lines:
			if "*NAME*" in line:
				...
				line = line.replace("*YMIN*", "{}".format(int(a[0]))) #* HEIGHT))) # we dont need this as we already have it scaled
